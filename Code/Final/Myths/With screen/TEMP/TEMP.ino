...
void loop() {
  if (digitalRead(switchPin) == LOW) {  // If the switch is pressed
    ...
    if (millis() - pirLastTriggered < pirHoldTime) {
      ...
      Display.beginDraw();
      Display.image(lasthumidity < 13 ? img_test : (lasthumidity >= 13 && lasthumidity <= 26 ? img_test2 : img_test3), 75, 50);
      Display.endDraw();
      servoOne.writeMicroseconds(lasthumidity < 13 ? 1000 : 1500);
      servoTwo.writeMicroseconds(lasthumidity >= 13 && lasthumidity <= 26 ? 1600 : 1500);
      servoThree.writeMicroseconds(lasthumidity > 26 ? 1400 : 1500);
    } else {
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (strcmp(topic, "student/EM1/zczqgch/humidity") == 0) {
    ...
    int targetAngle = lasthumidity < 13 ? 0 : (lasthumidity >= 13 && lasthumidity <= 26 ? 80 : 160);
