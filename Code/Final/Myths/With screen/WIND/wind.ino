...
int lastwindspeed = 0;        // Stores the last windspeed value
...
void loop() {
  if (digitalRead(switchPin) == LOW) {  // If the switch is pressed
    ...
    if (millis() - pirLastTriggered < pirHoldTime) {
      // Controls other servos based on windspeed
      Display.beginDraw();
      Display.image(lastwindspeed < 3.4 ? img_test : (lastwindspeed >= 3.4 && lastwindspeed <= 10.8 ? img_test2 : img_test3), 75, 50);
      Display.endDraw();
      servoOne.writeMicroseconds(lastwindspeed < 3.4 ? 1450 : 1500);
      servoTwo.writeMicroseconds(lastwindspeed >= 3.4 && lastwindspeed <= 10.8 ? 1200 : 1500);
      servoThree.writeMicroseconds(lastwindspeed > 10.8 ? 1800 : 1500);
    } else {
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  // If the message is related to windspeed
  if (strcmp(topic, "student/EM1/zczqgch/windspeed") == 0) {
    lastwindspeed = atoi(message);  // Converts the message to an integer
    int targetAngle = lastwindspeed < 3.4 ? 0 : (lastwindspeed >= 3.4 && lastwindspeed <= 10.8 ? 80 : 160);
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect(clientId.c_str(), mqtt_user, mqtt_password)) {
      ...
      client.subscribe("student/EM1/zczqgch/windspeed", 1);  // Subscribes to the windspeed topic
      client.loop();
    } else {
