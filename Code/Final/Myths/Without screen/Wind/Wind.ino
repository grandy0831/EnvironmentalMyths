...
Servo servoOne;    // First servo
Servo servoTwo;    // Second servo
Servo servoThree;  // Third servo
...
int lastWindSpeed = 0;  // Stores the last wind speed value
...
void callback(char* topic, byte* payload, unsigned int length) {
    ...
    // If the message is related to wind speed
    if (strcmp(topic, "student/EM/zczqgch/windspeed") == 0) {
        lastWindSpeed = atoi(message); // Converts the message to an integer
        int targetAngle = lastWindSpeed < 3 ? 0 : (lastWindSpeed >= 3 && lastWindSpeed <= 6 ? 80 : 160);
...
void reconnect() {
    ...
    while (!client.connected()) {
        ...
        if (client.connect(clientId.c_str(), mqtt_user, mqtt_password)) {
            ...
            client.subscribe("student/EM/zczqgch/windspeed", 1); // Subscribes to the wind speed topic
        } else {
...
void loop() {
    if (digitalRead(switchPin) == LOW) { // If the switch is pressed
        ...
        if (millis() - pirLastTriggered < pirHoldTime) {
            // Controls other servos based on wind speed
            servoOne.writeMicroseconds(lastWindSpeed < 3 ? 1000 : 1500);
            servoTwo.writeMicroseconds(lastWindSpeed >= 3 && lastWindSpeed <= 6 ? 1000 : 1500);
            servoThree.writeMicroseconds(lastWindSpeed > 6 ? 1000 : 1500);
        } else {
