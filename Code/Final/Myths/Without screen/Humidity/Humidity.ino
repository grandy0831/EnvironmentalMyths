...
void callback(char* topic, byte* payload, unsigned int length) {
    ...
    if (strcmp(topic, "student/EM/zczqgch/humidity") == 0) {
...
void reconnect() {
    ...
    while (!client.connected()) {
        ...
        if (client.connect(clientId.c_str(), mqtt_user, mqtt_password)) {
            ...
            client.subscribe("student/EM/zczqgch/humidity", 1); // Subscribes to the humidity topic
        } else {
...
void loop() {
    if (digitalRead(switchPin) == LOW) { // If the switch is pressed
        ...
        if (millis() - pirLastTriggered < pirHoldTime) {
            ...
            servoOne.writeMicroseconds(lastHumidity < 30 ? 1000 : 1500);
            servoTwo.writeMicroseconds(lastHumidity >= 30 && lastHumidity <= 60 ? 1000 : 1500);
            servoThree.writeMicroseconds(lastHumidity > 60 ? 1000 : 1500);
        } else {
